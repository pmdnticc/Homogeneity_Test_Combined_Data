...
C
    2 IF (N.EQ.1)            RETURN
      IF (MODE)    10,20,30
   10 CALL SORTTI (int(A),INDEX,N)
      GO TO 40
C
   20 CALL SORTTC(int(A),INDEX,N)
      GO TO 40
C
...
c$$$c **********  THIS IS NOT A CERN ROUTINE!!  ***************
c$$$      logical function pwhg_isfinite(x)
c$$$      implicit none
c$$$      include 'nlegborn.h'
c$$$      include 'pwhg_kn.h'
c$$$      real * 8 x
c$$$c According to ieee standards, a NaN is the only real not
c$$$c satisfying x.eq.x.
c$$$      if (.not.(x.eq.x)) then
c$$$         pwhg_isfinite = .false.
c$$$         if(kn_jacborn.ne.0) then
c$$$            call increasecnt('NaN exception')
c$$$         else
c$$$            call increasecnt('NaN exception with kn_jacborn=0')
c$$$         endif
c$$$         return
c$$$      endif
c$$$c Put constraint to avoid denormals
c$$$      if(x.gt.1.or.x.lt.-1) then
c$$$         if (1/x.eq.0) then
c$$$            pwhg_isfinite = .false.
c$$$            call increasecnt('Inf exception')
c$$$            return
c$$$         endif
c$$$      endif
c$$$      pwhg_isfinite = .true.
c$$$      end

c$$$      subroutine initcnt
c$$$      implicit none
c$$$      integer maxnum
c$$$      parameter (maxnum=100)
c$$$      logical ini
c$$$      character * 100 keywords(maxnum)
c$$$      real * 8 counters(maxnum)
c$$$      integer ncounters
c$$$      common/ccounters/keywords,counters,ncounters
c$$$      data ini/.true./
c$$$      save ini
c$$$      if(ini) then
c$$$         ncounters=0
...
c$$$      end
c$$$
c$$$      subroutine increasecnt(string)
c$$$      implicit none
c$$$      character *(*) string
c$$$      integer maxnum
c$$$      parameter (maxnum=100)
...
c$$$      call initcnt
c$$$      do j=1,ncounters
c$$$         if(string.eq.keywords(j)) then
c$$$            counters(j)=counters(j)+1
c$$$            return
c$$$         endif
c$$$      enddo
c$$$c not found
c$$$      if(ncounters.eq.maxnum) then
c$$$         write(*,*) ' increasecnt: too many counters requested'
c$$$         write(*,*) ' exiting ...'
c$$$         call exit(-1)
c$$$      endif
c$$$      ncounters=ncounters+1
c$$$      keywords(ncounters)=string
c$$$      counters(ncounters)=1
c$$$      end
c$$$
c$$$      subroutine addtocnt(string,value)
c$$$      implicit none
c$$$      character *(*) string
c$$$      real * 8 value
c$$$      integer maxnum
...
c$$$      do j=1,ncounters
c$$$         if(string.eq.keywords(j)) then
c$$$            counters(j)=counters(j)+value
c$$$            return
c$$$         endif
c$$$      enddo
c$$$c not found
c$$$      if(ncounters.eq.maxnum) then
c$$$         write(*,*) ' addtocnt: too many counters requested'
c$$$         write(*,*) ' exiting ...'
c$$$         call exit(-1)
c$$$       endif
c$$$      ncounters=ncounters+1
c$$$      keywords(ncounters)=string
c$$$      counters(ncounters)=value
c$$$      end
c$$$
c$$$
c$$$      subroutine setcnt(string,value)
c$$$      implicit none
c$$$      character *(*) string
c$$$      real * 8 value
c$$$      integer maxnum
...
c$$$c not found
c$$$      if(ncounters.eq.maxnum) then
c$$$         write(*,*) 'setcnt: too many counters requested'
c$$$         write(*,*) ' exiting ...'
c$$$         call exit(-1)
c$$$      endif
c$$$      ncounters=ncounters+1
...
c$$$c not found
c$$$      if(ncounters.eq.maxnum) then
c$$$         write(*,*) ' resetcnt: too many counters requested'
c$$$         write(*,*) ' exiting ...'
c$$$         call exit(-1)
c$$$      endif
c$$$      ncounters=ncounters+1
c$$$      keywords(ncounters)=string
c$$$      counters(ncounters)=0
c$$$      end
c$$$
c$$$      subroutine incrcntrs(string,k)
c$$$      implicit none
c$$$      integer k
c$$$      character *(*) string
c$$$      character * 20 line
c$$$      line=string
c$$$      line(17:)=' '
c$$$      write(line(17:),'(i4)') k
c$$$      call increasecnt(line)
c$$$      end
c$$$
c$$$      subroutine incrcntrs2(string,j,k)
c$$$      implicit none
c$$$      integer j,k
c$$$      character *(*) string
c$$$      character * 20 line
...
c$$$     1     ,status='unknown')
c$$$      else
c$$$         stage=powheginput("#parallelstage")
c$$$         if(stage.gt.0) then
c$$$            write(cst(3:3),'(i1)') stage
c$$$            cst(1:2)='st'
c$$$            open(unit=iun,file=pwgprefix(1:lprefix)//'counters'//'-'
c$$$     1       //cst//'-'//rnd_cwhichseed//'.dat',status='unknown')
c$$$         else
c$$$            open(unit=iun,file=pwgprefix(1:lprefix)//'counters'//
...
c$$$      common/ccounters/keywords,counters,ncounters
c$$$      integer j,k
c$$$      if(ncounters.eq.0) return
c$$$      write(iun,*)
c$$$      write(iun,*) 'Value of counters at end of run:'
c$$$      do j=1,ncounters
c$$$         k=100
c$$$ 1       if(keywords(j)(k:k).eq.' ') then
c$$$            k=k-1
c$$$            goto 1
c$$$         endif
c$$$         write(iun,*) keywords(j)(1:k),' = ',counters(j)
c$$$      enddo
c$$$      write(iun,*)
c$$$      end
c$$$
c$$$      function getcnt(string)
c$$$      implicit none
c$$$      real * 8 getcnt
c$$$      character *(*) string
c$$$      integer maxnum
...
c$$$
c$$$
c$$$      subroutine reset_timer
c$$$      implicit none
c$$$      integer nmax
c$$$      parameter (nmax=10)
c$$$      integer n,i1(nmax),i2(nmax),i3(nmax)
c$$$      common/pwhg_cmytimer/n,i1,i2,i3
c$$$      save /pwhg_cmytimer/
c$$$      logical ini
c$$$      data ini/.true./
c$$$      save ini
c$$$      if(ini) then
...
c$$$      endif
c$$$      if(n.ge.nmax) then
c$$$         write(*,*) ' more than ',nmax,'reentrant calls of reset_timer'
c$$$         write(*,*) ' increase the parameter nmax in reset_timer'
c$$$         call pwhg_exit(-1)
c$$$      endif
c$$$      n=n+1
c$$$      call system_clock(i1(n),i2(n),i3(n))
c$$$      end
c$$$
c$$$      subroutine get_timer(seconds)
c$$$      implicit none
c$$$      real * 8 seconds
c$$$      integer j1,j2,j3
c$$$      integer nmax
...
c$$$      if(n.le.0) then
c$$$         write(*,*) ' ************ ERROR ******************'
c$$$         write(*,*) ' get_timer called with no previous set_timer call'
c$$$         call pwhg_exit(-1)
c$$$      endif
c$$$c j1: seconds*j2;
c$$$c j3: max value of j1; after that j1 restart from zero
c$$$c With gfortran, j3 corresponds roughly to 24 days.
c$$$      call system_clock(j1,j2,j3)
c$$$c the whole point is to make sure we are not wrapping around
c$$$      if(j1.ge.i1(n)) then
c$$$c No wrap around
c$$$         seconds=dble(j1-i1(n))/i2(n)
c$$$      else
c$$$c Assume we wrapped around
c$$$         seconds=dble(j1)/i2(n)+dble(i3(n)-i1(n))/i2(n)
c$$$      endif
c$$$      n=n-1
c$$$      end
...
c$$$      subroutine pwhg_exit(iret)
c$$$      integer iret
c$$$      real * 8 tmp,powheginput
c$$$      tmp=powheginput('print unused tokens')
